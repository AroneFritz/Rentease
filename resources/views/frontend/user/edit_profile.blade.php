    ...
    >
      <div class="container">
        <div class="breadcrumb-content text-center">
          <h1>{{ __('Perosonal Information') }}</h1>
          <ul class="list-inline">
            <li><a href="{{ route('index') }}">{{ __('Home') }}</a></li>
            <li><i class="far fa-angle-double-right"></i></li>
            <li>{{ __('Perosonal Information') }}</li>
          </ul>
        </div>
      </div>
    </section>
                    ...
                    <div class="edit-info-area">
                      <form action="{{ route('user.update_profile') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="row">
                          ...
                          <div class="col-lg-6">
                            <input type="text" class="form_control" placeholder="{{ __('Full Name') }}" name="username" value="{{ $userInfo->username }}">
                            @error('username')
                              <p class="mb-3 ml-2 text-danger">{{ $message }}</p>
                            @enderror
                          </div>
                          ...
                          <div class="col-lg-6">
                            <input type="text" class="form_control" placeholder="{{ __('Province') }}" name="state" value="{{ $userInfo->state }}">
                            @error('state')
                              <p class="mb-3 ml-2 text-danger">{{ $message }}</p>
                            @enderror
                          </div>
