...
class VendorController extends Controller
{
    ...
    {
        // Update validation rules to expect email instead of username
        $rules = [
        ...
        // Attempt login with email instead of username
        if (Auth::guard('vendor')->attempt([
            'email' => $request->email,  // Use email instead of username
            ...
            // Check if email is verified and status is active
            if ($setting->vendor_email_verification == 1 && $authAdmin->email_verified_at == NULL && $authAdmin->status == 0) {
                ...
                // Logout if email verification condition is not satisfied
                Auth::guard('vendor')->logout();
                Session::forget('secret_login');

                return redirect()->back();
            } elseif ($setting->vendor_email_verification == 0 && $setting->vendor_admin_approval == 1) {
                Session::flash('success', 'Welcome! You have successfully logged in as a landlord.');
